...
class VB_WebBanking
  ...
  def check_accounts
    ...
    @accounts = Array.new
    accounts_html.css('div[class="contracts-section"]').map do |i|
      unless i.css('div[class="section-title no-data-error"]').any?
        name      = i.css('div[class="main-info"]').css('a[class="name"]').text
        balance   = i.css('div[class="primary-balance"]').css('span.amount').first.text
        currency  = i.css('div[class="primary-balance"]').css('span.amount').last.text
  ...
  def check_transactions
    ...
    transactions_html.css('li[class="history-item success "]').map do |i|
      year  = i.xpath('../../preceding-sibling::div[@class = "month-delimiter"]').last.text.split[1]
      month = i.xpath('../../preceding-sibling::div[@class = "month-delimiter"]').last.text.split[0]
      day   = i.parent.parent.css('div[class = "day-header"]').text.split[0]
      time  = i.css('span.history-item-time').text
      date  =  Date.parse(year + " " + month + " " + day).to_s
      description = i.css('span.history-item-description').text.split.join(" ")
      if not i.css('span.history-item-amount.transaction.income').text.empty?
        amount = i.css('span.history-item-amount.transaction.income').text
      elsif not i.css('span.history-item-amount.total').text.empty?
        amount = i.css('span.history-item-amount.total').text
      else
        amount = i.css('span.history-item-amount.transaction').text
      end
